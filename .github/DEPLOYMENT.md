...
`main` 또는 `deploy` 브랜치에 다음 경로의 파일이 변경되면 자동 배포:
- `src/**`
- `build.gradle`
- `Dockerfile`
...
```bash
# ECR 이미지 확인
aws ecr describe-images --repository-name sf-backend

# K8s Pod 상태 확인
kubectl get pods -n default -l app=factory-tycoon-backend

# 배포 로그 확인
kubectl logs -n default -l app=factory-tycoon-backend
```
...
cd factory-tycoon-k8s/helm/factory-tycoon-backend
